...
void storeEdge(void)
{
    ...
    ed[i].y2 = ed[0].y1;
    ed[i].flag = 0;
    cout << "finished STORING\n";
}

void sortEdge(void)
{
    cout << "started SORTING\n";
    /*Check for y1>y2, if not interchnge y1 and y2 */
    //edges are getting sorted by maxY
    for (i = 0; i < n; i++)
    {
        if (ed[i].y1 < ed[i].y2)
        {
...
void scanLine(void)
{
    ...
    while (yy > ymin)
    {
        ...
        for (i = 0; i < j; i = i + 2)
        {
            ...
            glBegin(GL_LINES);
            glVertex2f((int)x_int[i], yy);
            glVertex2f((int)x_int[i + 1], yy);
            glEnd();
            glFlush();
        }
        yy--;
    }
    cout << "stopped FILLING\n";
}
...
void drawPolygon(void)
{
    ...
    {
        glBegin(GL_LINES);
        glVertex2f(ed[i].x1, ed[i].y1);
        glVertex2f(ed[i].x2, ed[i].y2);
        glEnd();
        glFlush();
